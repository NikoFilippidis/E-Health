...
namespace eRezeptRCF{
	public static class eRezept_API{
		...
		public static void saveERezeptOnCard(ERezept er){
			String path = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
			path+="\\Karte";
			if(!Directory.Exists(path))
				Directory.CreateDirectory(path);
			StreamWriter file = new StreamWriter(path+"\\"+er.rezeptNR+"eRezept_"+er.datum.ToString("dd-MMM-yy"));
			file.WriteLine("<EREZEPT>");
			file.WriteLine(LocalDoctor.getExportFormat());
			file.WriteLine("<SPLIT>");
			file.WriteLine(er.getExportFormat());
			file.WriteLine("<SPLIT>");
			file.WriteLine(SecurityHelper.MD5HashText(er.getExportFormat()));
			file.WriteLine("<EREZEPT>");
			file.Close();
		}
